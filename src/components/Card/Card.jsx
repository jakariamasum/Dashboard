...
const Card = ({colour,tag,text,progress}) => {
    const textStyle={
        color:colour,
    }
    return (
        <div className='p-8 w-[310px] bg-[#211A75] text-white rounded-xl'>
            <div className='flex justify-between items-center'>
                <div className='flex items-center'>
                    <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 10 10" fill="none">
                        <circle cx="5" cy="5" r="5" fill={colour} />
                    </svg>
                    <h1 className={`ml-2`} style={textStyle}>{tag}</h1>
                </div>
                <div>
            ...
            <h1 className='text-[18px] font-semibold mb-5'>{text}</h1>
            <div style={{ width: '258px', backgroundColor: '#1E1C3A' }}>
                <ProgressBar progressValue={progress} progressColor={colour} />
            </div>
            <div className='mt-5 flex justify-between items-center'>
                ...
                <div className='flex items-center'>
                    ...
                    <p className='ml-2'>Due in 4 days</p>
                </div>
            </div>
        </div>
    );
